...
export default function CalendarPage() {
  ...
  if (!connected) {
    return (
      <div className="flex flex-col items-center justify-center h-full px-4 text-center py-32">
        ...
        <p className="text-gray-400 mb-8 max-w-md text-lg">Please connect your wallet to view your scheduled yield distributions and payout calendar.</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-4 sm:p-6 md:p-8 w-full max-w-[1200px] mx-auto space-y-8 min-w-0 pb-24 relative">
      ...
      {/* HEADER SECTION */}
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-6 relative z-10 border-b border-white/10/50 pb-8">
        <div>
          ...
          <p className="text-base text-gray-400 mt-2 max-w-md">Your scheduled yield distributions across all active BizSwap instruments.</p>
        </div>

        {/* Legend */}
        <div className="flex items-center gap-4 bg-[#0A0F17]/80 backdrop-blur-md px-5 py-3 rounded-2xl border border-white/10 shadow-inner">
          <span className="flex items-center gap-2 text-[11px] font-bold text-gray-400 uppercase tracking-wider"><div className="w-2.5 h-2.5 rounded-full bg-[#FF6B6B] shadow-[0_0_10px_rgba(255,107,107,0.5)]"></div> BizYield</span>
          <span className="flex items-center gap-2 text-[11px] font-bold text-gray-400 uppercase tracking-wider"><div className="w-2.5 h-2.5 rounded-full bg-[#3B82F6] shadow-[0_0_10px_rgba(59,130,246,0.5)]"></div> BizCredit</span>
          <span className="flex items-center gap-2 text-[11px] font-bold text-gray-400 uppercase tracking-wider"><div className="w-2.5 h-2.5 rounded-full bg-[#81D7B4] shadow-[0_0_10px_rgba(129,215,180,0.5)]"></div> BizBond</span>
        </div>
      </div>
      ...
      {!loading && events.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 md:gap-6 relative z-10">
          <div className="bg-gradient-to-br from-[#121A27] to-[#0A0F17] border border-white/10 rounded-2xl p-6 shadow-xl relative overflow-hidden group">
            ...
            <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mb-1 relative z-10">Next Payout</p>
          ...
          <div className="bg-gradient-to-br from-[#121A27] to-[#0A0F17] border border-white/10 rounded-2xl p-6 shadow-xl relative overflow-hidden group">
            ...
            <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mb-1 relative z-10">30-Day Outlook</p>
            <div className="flex items-baseline gap-1 relative z-10">
          ...
          <div className="bg-gradient-to-br from-[#121A27] to-[#0A0F17] border border-white/10 rounded-2xl p-6 shadow-xl relative overflow-hidden group">
            ...
            <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mb-1 relative z-10">Total Scheduled</p>
            <div className="flex items-baseline gap-1 relative z-10">
      ...
      {/* TIMELINE */}
      {loading ? (
        <div className="relative border-l-2 border-white/10 ml-4 md:ml-8 space-y-8 pb-12 mt-8 z-10">
          {[1, 2, 3].map((i) => (
            <div key={i} className="relative pl-8 md:pl-12">
              <div className="absolute -left-[11px] top-6 w-5 h-5 rounded-full border-4 border-[#0A0F17] bg-[#1C2538]"></div>
              <div className="bg-[#0A0F17] border border-white/10 p-6 rounded-2xl flex flex-col md:flex-row items-start md:items-center gap-6 animate-pulse">
        ...
        <div className="p-16 flex flex-col items-center justify-center text-center mt-8 bg-[#0A0F17]/50 border border-white/10 rounded-3xl backdrop-blur-md relative z-10">
          <div className="w-20 h-20 rounded-full bg-[#1C2538]/30 flex items-center justify-center mb-4 border border-white/20/30">
            <Calendar01Icon className="w-8 h-8 text-[#4B5A75]" />
          </div>
          <h3 className="text-xl font-black text-[#F9F9FB] mb-2">No Scheduled Payments</h3>
          <p className="text-gray-400 max-w-sm mb-6">You don't have any upcoming yield distributions. Purchase BizShares to start earning.</p>
              ...
              <div key={i} className="relative pl-8 md:pl-12 group">
                ...
                <div className={`relative overflow-hidden ${cardBg} border ${cardBorder} ${isFirst ? 'shadow-[0_8px_32px_rgba(0,0,0,0.3)] backdrop-blur-xl' : 'shadow-lg hover:border-white/20 hover:shadow-xl hover:-translate-y-0.5'} p-5 sm:p-6 rounded-2xl flex flex-col md:flex-row justify-between items-start md:items-center gap-6 transition-all duration-300`}>
                  ...
                  <div className="flex items-center gap-4 sm:gap-6">
                    <div className="text-center w-14 sm:w-16 flex-shrink-0 bg-[#0A0F17]/80 backdrop-blur-sm py-2 sm:py-3 px-2 sm:px-3 rounded-xl border border-white/10 shadow-inner">
                      ...
                      <p className={`text-xl sm:text-2xl font-black ${isFirst ? 'text-[#F9F9FB]' : 'text-gray-400'}`}>{ev.date.getDate()}</p>
                    </div>
                    <div>
                      ...
                      <p className="text-[11px] sm:text-xs text-gray-400 font-bold uppercase tracking-wider">Scheduled Yield Distribution</p>
                    </div>
                  </div>
                  
                  <div className="flex items-center justify-between md:justify-end gap-6 w-full md:w-auto border-t border-white/10/50 md:border-0 pt-4 md:pt-0 mt-2 md:mt-0">
