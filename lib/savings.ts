...
export async function fetchSavingsDataForAddress(rawAddress: string, getPriceFn?: () => Promise<{ eth: number, gd: number }>) {
  if (!rawAddress) {
  ...
  try {
    ...
    if (getPriceFn) {
      try {
        const prices = await getPriceFn();
        currentEthPrice = prices.eth;
        goodDollarPrice = prices.gd;
      } catch (e) {
        console.warn("Failed to fetch internal prices");
      }
    }
