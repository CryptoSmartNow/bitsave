...
export async function sendBizSwapMintEmail(email: string, instrument: string, assetName: string, investmentAmount: number, payoutFrequency: string, apr: string, certificateId: string) {
  if (!email || !process.env.SMTP_USER || !process.env.SMTP_PASSWORD) return;
